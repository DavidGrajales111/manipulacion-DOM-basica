...
function presentacion(completeName, nickName) {
    console.log("Hola mi nombre es " + completeName + " ,pero prefiero que me llamen " + nickName + ".");
}
```
...
```js Esta es mi manera, es importante saber que no no escribes la palabra exsactamente como dice ahí no funciona (Igual JavaScript nos permite arreglar eso cambiando las mayusculas o minisculas de loq ue el usuario escriba en el promt, pero aun no se como hacer eso.)

let tipoDeSuscripcion = String(prompt("Cual es su tipo de suscripción de Platzi: Basic, Experto o ExpertPlus"))
